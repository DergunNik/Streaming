...
public class VideoService : VoD.VideoService.VideoServiceBase
{
    ...
    public VideoService(Cloudinary cloudinary, AppDbContext dbContext)
    {
    ...
    public override async Task<GetVideosOfUserReply> GetVideosOfUser(GetVideosOfUserRequest request, ServerCallContext context)
    {
        ...
        {
            searchBuilder = searchBuilder.NextCursor(request.PageToken);
        }
        ...
        ServerCallContext context)
    {
        ...
        var reaction = await _dbContext.Reactions.FirstOrDefaultAsync(
            r => r.PublicId == request.PublicId && r.UserId == request.UserId, 
        ...
        if (reaction is null && request.Reaction != ReactionType.ReactionUnspecified)
        {
            ...
            if (newReaction.IsLike) { video.LikeCount++; }
                   ...
                   request.Reaction == ReactionType.Dislike && !reaction.IsLike))
        {
            switch (request.Reaction)
            {
                ...
                case ReactionType.ReactionUnspecified:
                    ...
                    if (reaction.IsLike) { video.LikeCount--; }
                    else { video.DislikeCount--; }
                    break;
